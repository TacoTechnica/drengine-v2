...
namespace GameEngine.Game.UI
{
    public class UITextInput : UITextInputBase
    {
        ...
        private readonly UIColoredRect _selectRect;
        ...
        public UITextInput(GamePlus game, Font font, Color color, UIComponent parent = null) : base(game, parent)
        {
            ...
            _selectRect = (UIColoredRect) new UIColoredRect(game, SelectColor, false, this)
                .WithLayout(Layout.CornerLayout(Layout.TopLeft));
        ...
        protected override void Draw(UIScreen screen, Rect targetRect)
        {
            ...
            var sub = Text.Substring(0, _cursorPos);
            try
            {
                var delta = _text.GetSize(sub);
                var cursorSize = new Vector2(2, _text.Font.SpriteFont.LineSpacing);
        ...
        protected override void OnControlInput(char c)
        {
            ...
            switch (c)
            {
                ...
                case 'v':
                    ...
                    var toPaste = ClipboardService.GetText();
                    InsertText(toPaste);
                ...
                case 'x':
                    if (Selecting)
                    {
                        var toCopy = Text.Substring(SelectBegin, SelectEnd - SelectBegin);
        ...
        protected override void OnBackspace(bool ctrl)
        {
            ...
            else
            {
                ...
                if (ctrl)
                {
                    var lastEndWord = GetPrevControlSplit();
                    if (lastEndWord == -1)
                        count = _cursorPos;
                    else
        ...
        {
            var newPos = _cursorPos - 1;
            if (ctrl) newPos = GetPrevControlSplit();
        ...
        {
            var newPos = _cursorPos + 1;
            if (ctrl) newPos = GetNextControlSplit();
        ...
        private void SelectChange(int prevPosition, int newPosition)
        {
            ...
            else if (SelectBegin < newPosition && newPosition < SelectEnd)
            {
                ...
                {
                    Debug.LogError(
                        $"Invalid selection range: [{SelectBegin}, {SelectEnd}] for transition from {prevPosition} => {newPosition}");
                    SelectBegin = -1;
                    SelectEnd = -1;
                }
            }
        ...
        private void UpdateSelectVisual()
        {
            Vector2 startPos = _text.TextMin + _text.GetSize(Text.Substring(0, SelectBegin)),
                endPos = _text.TextMin +
                         _text.GetSize(Text.Substring(0, SelectEnd)); //SelectBegin, SelectEnd - SelectBegin));
        ...
        {
            var ind = Text.Substring(0, _cursorPos).LastIndexOfAny(new[] {' ', ',', '.', '_'});
            if (ind == -1) return 0;
            return ind;
        }
        ...
        private int GetNextControlSplit()
        {
            ...
            var ind = Text.IndexOfAny(new[] {' ', ',', '.', '_'}, _cursorPos + 1);
            if (ind == -1) return Text.Length;
            return ind;
        }
    }
}