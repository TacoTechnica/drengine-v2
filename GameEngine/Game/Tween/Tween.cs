...
namespace GameEngine.Game.Tween
{
    ...
    public abstract class Tween<T> : ITween
    {
        ...
        private readonly Action<T> _onTween;
        ...
        private readonly Func<float, T> _tweenFunction;
        ...
        public bool RunUpdate()
        {
            ...
            var timePassed = GetTime() - _startTime;
            if (timePassed > _delay)
            {
                timePassed -= _delay;

                var progress = Math.Clamp01(timePassed / _duration);
                ...
                if (_useOvershooting)
                {
                    if (progress > 1) progress = progress - (progress - 1) / (_overshoot + 1);
                }
                else
                {
                ...
                var value = _tweenFunction(progress);
                _onTween.Invoke(value);

                Update(value);

                if (timePassed > _duration) Finish();
            }
        ...
        {
            if (_useUnscaledTime) return _parent.Game.UnscaledTime;
