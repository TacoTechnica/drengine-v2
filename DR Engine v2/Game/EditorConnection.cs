...
namespace DREngine.Game
{
    ...
    public class EditorConnection : DRConnection
    {
        ...
        public EditorConnection(bool active, string editorPipeReadHandle, string editorPipeWriteHandle) : base(active? new AnonymousPipeClientStream(PipeDirection.In, editorPipeReadHandle) : null, active? new AnonymousPipeClientStream(PipeDirection.Out, editorPipeWriteHandle) : null)
        {
            Active = active;
            if (active)
            {
                ...
                Debug.OnLogError += (message, stacktrace) => { SendCommand(NetworkHelper.ERROR_COMMAND, message + " : " + stacktrace); };
            }
        }
